...
const historyAPI = {
  get: ({ text, startTime, endTime, maxResults, callback }) => {
    ...
    if (!chrome.history) {
      ...
      chrome.history.search({ text, startTime, endTime, maxResults }, (historyItemList) => {
        historyItemList = historyItemList.filter(
          (historyItem) =>
            startTime <= historyItem.lastVisitTime && historyItem.lastVisitTime <= endTime
        );
        historyItemList.sort(
          (preHistory, curHistory) => curHistory.lastVisitTime - preHistory.lastVisitTime
        );
        let prevDate = '';
